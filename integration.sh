...
# once. There is an MR per bump, but nothing pauses for anyone to read it.
# CHRIS HAS ACCEPTED THAT (2026-09-13), automerge and all — it is the arrangement
# he wants, not an oversight to raise again. What it changes is what a build IS:
# treat it as a release to one production CA rather than as a canary, and report
# the digest after a push, not just that it happened.
#
# Rebuilds force-push this branch, so the previous tip stops being reachable
# from any ref. That was reported as breaking the deployment's Flux reconcile,
# which pins branch+commit — but Chris says it has never actually failed: the
# commit objects stick around after the branch moves, so the pin still resolves.
# The mechanism is real and the consequence was not, so do not re-derive it.
#
# ---------------------------------------------------------------------------
...
# usually merge cleanly. Keep any pair that truly conflicts adjacent, so it
# surfaces once.
#
# Measure that in two steps, never as one pairwise `merge-tree` against main. A
# branch can sit hundreds of commits behind main, and `--merge-base=origin/main`
# then reads everything main gained since as that branch's own deletion, so the
# branch is reported as conflicting with every other entry. Merge each branch
# onto main FIRST, then merge-tree those results against each other: both sides
# carry main, so the base is main and what is left is the genuine pair.
# integration-trial.sh is already safe here — it accumulates, so every step's
# merge base is real.
#
# ---------------------------------------------------------------------------
...
BRANCHES=(
  ...
  # Both of these collide with the whole managed-certificate stack below, so
  # they sit against it: #312 on the docs/development/locking.md lock-name
  # table, #371 on charts/openvox-ca/values.yaml and internal/storage/sql.go
  # (and, with #344 only, cmd/openvox-ca/main.go).
  #
  # The locking.md collision is resolved ROW BY ROW: take whichever side carries
  # the newer claim for each row rather than picking a side. All three stack
  # members inherit the same rows, so one resolution serves all three.
  origin/feature/188-rebuild-inventory-hmac   # PR #312 — issue #188
  origin/fix/351-sqlite-database-permissions  # PR #371 — issue #351

  # A STACK, in ancestry order — each is an ancestor of the next, so the order
  # is forced rather than chosen: #322 -> #336 -> #344.
  ...
  # #282 sits between the two sides it collides with: #336 and #344 above
  # (README.md, packaging/systemd/openvox-ca.service) and #266 below
  # (magefile.go).
  #
