...
type distVariantSpec struct {
	...
	// oversight -- flip this to true and release.yml's package counts must move
	// with it, which is what verifyDistVariants enforces.
	packaged bool
}
...
// implemented #250, since nothing here could enforce it; #282 is that
// implementation and satisfies it. A second list is exactly the drift the
// packaged field was made a field to avoid, and the two ways it goes wrong are
// not symmetric. A format added here alone forces release.yml to name it at
// every site and then fails a tag with "Expected 2 release apk packages, found
// 0" -- loud, and precisely the failure this guard exists to move earlier. A
// format added to the producer alone is built into dist/ and silently dropped,
// because the upload list globs only the declared extensions.
//
