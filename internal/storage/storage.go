...
func (s *StorageService) InventoryIntegrityReport(ctx context.Context) (InventoryIntegrityReport, error) {
	...
	if chained && len(entries) == 0 {
		...
		rep.LegacyUndecomposed = len(parseInventoryBlob(legacy)) > 0
	}
...
// RebuildConfirmFlag is the flag name the offline rebuild requires before it
// will act. Exported and consumed by cmd/openvox-ca rather than spelled twice:
// this package names it in the hint an operator reads out of a log line, and
// the command defines the flag itself, so a rename in one place would otherwise
// leave the other pointing at a flag that does not exist.
const RebuildConfirmFlag = "yes-re-bless"
...
const inventoryRepairHint = "openvox-ca rebuild-inventory-hmac reports the state and, with " +
	"--" + RebuildConfirmFlag + ", re-asserts integrity over the inventory as it stands"
