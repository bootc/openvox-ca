...
var _ = Describe("CRL chain read failures", func() {
	...
	It("counts a CRL lock it could not take, on every writer that takes it", func() {
		...
		// The fifth, added after review found it outside the enumeration while
		// its four siblings were inside it. RevokeSerial is the
		// openvox-ca-ctl revoke --serial path and the only way to retire a
		// superseded certificate -- an operator-requested revocation, so the
		// same rule that exempts the best-effort sites requires counting this
		// one. The serial need not exist: the lock is refused before the closure
		// runs, which is the arm under test, so anything NormaliseSerial accepts
		// gets there.
		Expect(myCA.RevokeSerial(ctx, "DEADBEEF", false)).NotTo(Succeed())
		Expect(myCA.CRLUpdateFailures()).To(Equal(before+5), "RevokeSerial")
	})
	...
	// the five writers above. Wrapping it in withCRLLockCounted would move this
	...
	It("does not count a lock AutoRenew could not take for its best-effort revoke", func() {
		// The third of the three sites whose comments all say "deliberately not
		// withCRLLockCounted" and justify it identically. Clean and Renew were
		// pinned; this one was not, so the enumeration the other two establish
		// stopped one short -- and this is the site that runs unattended, on
		// every agent's own renewal timer, rather than on an operator's command.
		ctx := context.Background()
		dir := GinkgoT().TempDir()
		base := storage.NewFilesystemBackend(dir)
		store := storage.NewWithBackend(base, filepath.Join(dir, "private"))
		...
		myCA.CAKeyConfig = ca.KeyConfig{Algo: ca.KeyAlgoECDSA, Size: 256}
		Expect(myCA.Init(ctx)).To(Succeed())
		cert := signedCert(myCA, "autorenewme.test")

		// Only the CRL lock is refused, as for Renew: AutoRenew holds the
		// subject lock already, and refusing that would fail before this arm.
		refusing := &crlLockRefusingBackend{Backend: base}
		myCA.Storage = storage.NewWithBackend(refusing, filepath.Join(dir, "private"))

		before := myCA.CRLUpdateFailures()
		Expect(myCA.AutoRenew(ctx, cert)).Error().NotTo(HaveOccurred(),
			"the renewal stands; revoking the replaced certificate is best-effort")
		Expect(refusing.refused).To(BeNumerically(">", 0),
			"the fixture must actually have refused the CRL lock; if lockNameCRL is "+
				"renamed it grants everything and the assertion below means nothing")
	...
	It("counts a lock the refresh pass could not take as a CRL-update failure", func() {
		...
		// Exactly one, like every sibling in this block. RefreshCRLChainFile
		// guards specifically against counting a failure twice -- once on its
		// own error arm and again beneath signCRLLocked, which is what the
		// countedByCRLLayer flag exists for -- and that is the property a `>`
		// comparison cannot see, since 1, 2 and any larger delta all satisfy it.
		// This is the only spec that drives this arm at all, so a regression
		// that made it double-count would otherwise pass in silence.
		Expect(myCA.CRLUpdateFailures()).To(Equal(before+1),
			"a refresh that could not take the lock amended nothing, and must be counted exactly once")
