...
A subject the inventory has no entry for is a `404 Not Found`, with a body naming it, matching Puppet Server and the [by-serial route's `404`](#revocation-by-serial). It is the only `404` this arm produces, and everything else — including a CRL this CA cannot read — stays a `409`. Usually it means a mistyped certname, but do not read it as proof of one: an inventory that has gone missing answers the same way, on every backend, which is why the body says only that there is no inventory entry rather than that nothing was ever issued. The counter cannot separate those two for you — both are uncounted. On the [filesystem backend](storage-backends.md#filesystem-backend-default), the default, the log line the serving replica emits at info level, `No inventory entry for subject; revocation cannot proceed`, does separate them: its error attribute names the inventory path when the inventory and its integrity file are what is missing, and names no path when the subject is simply not listed. The structured backends answer the lookup from an index that verifies nothing, so they reach this `404` without an integrity check at all and the two cases arrive identical there; compare against `GET /certificate_statuses` (`openvox-ca-ctl list --all`) instead, which reads the certificate index rather than the inventory — a subject it still reports as `signed` is one whose inventory entry has gone, not whose certificate has.
