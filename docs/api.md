...
A subject the inventory has no entry for is a `404 Not Found`, with a body naming it, matching Puppet Server and the [by-serial route's `404`](#revocation-by-serial). That is the only `404` this arm produces, and it is deliberately narrow: every other failure, including one where the CA cannot read its own CRL, stays a `409`. The distinction is worth trusting, because it is the same one `puppetca_crl_update_failures_total` draws — a subject that was never issued is an operator typo and is not counted, while a read that *failed* is counted and answers `409`. So on this route the status code tells you which side of that counter you are on.
