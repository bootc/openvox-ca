...
A subject the inventory has no entry for is a `404 Not Found`, with a body naming it, matching Puppet Server and the [by-serial route's `404`](#revocation-by-serial). It is the only `404` this arm produces, and everything else — including a CRL this CA cannot read — stays a `409`. Usually it means a mistyped certname, but do not read it as proof of one: on the [filesystem backend](storage-backends.md#filesystem-backend-default) — the default — an inventory that has gone missing, along with its integrity file, answers the same way, which is why the body says only that there is no inventory entry rather than that nothing was ever issued. The counter cannot separate those two for you: both are uncounted. What separates them is the log line the replica that served the request emits, `No inventory entry for subject; revocation cannot proceed`, whose error attribute names the inventory path when the inventory is what is missing, and names no path when the subject simply is not listed.
