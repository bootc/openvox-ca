...
| `PUT` | `/certificate_status/{subject}` | Change state (`signed` or `revoked`); supports `cert_ttl` (seconds). `revoked` returns `404 Not Found`, with a body naming the subject, when the inventory has no entry for it and no request is queued for it; `409 Conflict` when the subject has only an unsigned CSR, matching Puppet Server, which reserves that code for exactly this case; and `409 Conflict` when the stored CRL was not signed by the CA certificate this process loaded — see the note below. `signed` returns `409 Conflict` when the queued CSR requests subject alternative names policy refuses ([`allow_subject_alt_names`](configuration.md#subject-alternative-names-requested-by-a-csr)) or extensions that disallow signing; both carry a body naming the cause |
...
One case that looks like it belongs with the `404` does not, and the split is deliberate: a subject holding **only an unsigned CSR** answers `409 Conflict` with a body saying so. It has no inventory entry either, so the CA reports the same absence internally — but Puppet Server reserves `404` for a name it has never heard of and `409` for a request that exists and is unsigned, and `puppetserver-ca-cli` branches on that, printing different messages and exiting `1` against `24`. Sign the request or [clean](#certificate-status) it; there is nothing to revoke until it is signed.
...
- **Empty body (wire-compatible auto-renewal):** matches the request real OpenVox/Puppet agents send by default (`hostcert_renewal_interval`, and the `puppet ssl renew_cert` CLI action). Identity and key possession come solely from the mTLS-presented client certificate; the same public key is reissued with a fresh serial and validity, carrying forward the original certificate's SANs, extended key usages and Puppet OID extensions unchanged. Unlike the CSR path, this **preserves authorization-arc OIDs** (e.g. `pp_cli_auth`): they were already vetted when the presented certificate was issued, so a cert that legitimately holds them keeps them across renewal. Extended key usages are carried forward for the same reason in the other direction: renewal must not return more than it was given, so a certificate issued with a narrow usage keeps that usage rather than gaining the default pair. A presented certificate carrying no extended key usage at all is unrestricted, and stays so — it receives the usual `serverAuth`, `clientAuth` default, since narrowing it on renewal would break agents issued before the CA set one.
