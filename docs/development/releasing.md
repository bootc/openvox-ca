...
specs assert both halves of that — that two builds at one epoch are identical,
and that a build at a different epoch differs, because the first alone is
satisfied by any two builds inside the same second.
