...
- **Pluggable storage:** filesystem (default, Puppet Server compatible), SQLite (single database file), or etcd / Redis (Valkey) for HA clusters; CA cert/key can be pinned to local files independently. See the [storage backends guide](docs/storage-backends.md)
